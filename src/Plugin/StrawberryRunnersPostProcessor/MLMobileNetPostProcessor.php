...
class MLMobileNetPostProcessor extends abstractMLPostProcessor {
  ...
  public function settingsForm(array $parents, FormStateInterface $form_state) {
    ...
    $element['ml_method'] = [
      ...
      '#options' => [
        '/image/mobilenet' => 'MobileNet (Image embeddings as a Unit Length Vector)',
      ],
  ...
  protected function runImageMLfromIIIF($io, NlpClient $nlpClient): \stdClass {
    ...
    $iiif_image_url =  $config['iiif_server']."/{$iiifidentifier}/{$iiif_image_url_region}/max/0/default.jpg";
    ...
    if (isset($ML['mobilenet']['objects']) && is_array($ML['mobilenet']['objects']) && count($ML['mobilenet']['objects']) > 0 ) {
      if (!empty($json_annotation)) {
        if (!$inherit_region) {
          ...
          $ML['mobilenet']['objects'] = array_slice($ML['mobilenet']['objects'] ?? [], 0, 1);
          $ML['mobilenet']['objects'][0]['bounding_box']['origin_x'] = ($json_annotation['x'] / $width);
          $ML['mobilenet']['objects'][0]['bounding_box']['origin_y'] = ($json_annotation['y'] / $height);
          $ML['mobilenet']['objects'][0]['bounding_box']['width'] = ($json_annotation['w'] / $width);
          $ML['mobilenet']['objects'][0]['bounding_box']['height'] = ($json_annotation['h'] / $height);
        }
      }
      $miniocr = $this->mobilenetToMiniOCR($ML['mobilenet']['objects'], $width, $height, $sequence_number);
      $annotations = $this->nativeDetectionToAnnotations($ML['mobilenet']['objects'], $width, $height);
    ...
    elseif (!empty($json_annotation) && isset($output->searchapi['vector_1024'])) {
      ...
      $ML['mobilenet']['objects'][0]['categories'][0]['category_name'] = $json_annotation['label'] . ' (inherited)';
      $ML['mobilenet']['objects'][0]['categories'][0]['score'] = $json_annotation['confidence'] . ' (inherited)';
  ...
  protected function nativeDetectionToAnnotations(array $objects, $width, $height) {
    ...
    foreach ($objects as $object) {
      ...
      $confidence_accumulative = 0;
      $text = '';
      foreach ($object['categories'] as $category) {
        $confidence_accumulative = $category['score'] ?? 0 +  $confidence_accumulative;
        $text .= (string)($category['category_name'] ?? 'Unlabeled') . ' ~ ' . (string)sprintf('%.3f', $category['score'] ?? 0);
      }
      $annotation = [];
      $annotation['label'] = "ML Mobilenet : " . $text;
      $annotation['confidence'] = is_array($object['categories']) && count($object['categories']) ? $confidence_accumulative/count($object['categories']) : 0;
      $annotation['model'] = 'mobilenet';
