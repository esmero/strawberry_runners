...
class StrawberryRunnersMLImageArgument extends SearchApiStandard {
  ...
  public function query($group_by = FALSE) {
    ...
    if (empty($this->expanded_argument) || ! $this->query) {
      // basically not validated, not present as a value and also someone cancelled/nulled the query before?
      return;
    }
    ...
    if ($plugin_config_entity->isActive()) {
      ...
      $cache_id = 'strawberry_runners_ml:arg:';
      $cache_suffix = $this->view->storage->id().':'. $this->view->current_display;
      $cache_arg = json_encode($this->value);
      $cache_id = $cache_id.md5($cache_suffix.$cache_arg);
      $cached = $this->cache->get($cache_id, FALSE);
      $response = [];
      if ($cached->data ?? NULL) {
        $response = (array) $cached->data ?? [];
      }
      if (empty($response)) {
        // Now we need to actually generate an instance of the runner using the config
        $sbr_config['configEntity'] = $entity_id;
