...
/**
 * Defines a filter that handles Text Similarity.
 *
 * @ingroup views_filter_handlers
 *
 * @ViewsFilter("sbr_textml_filter")
 */
class StrawberryRunnersMLTextfilter extends FilterPluginBase /* FilterPluginBase */ {
    ...
    foreach ($active_plugins as $by_source => $plugins) {
      foreach ($plugins as $entity_id => $active_plugin) {
        if (isset($active_plugin['ml_method'])) {
          if (in_array($active_plugin['ml_method'], array_keys(abstractMLPostProcessor::ML_TEXT_VECTOR_SIZE))){
  ...
  public function query() {
    ...
    if ($plugin_config_entity->isActive()) {
      ...
      if ($plugin_instance instanceof abstractMLPostProcessor) {
        try {
          $response = $plugin_instance->callTextML($this->value, TRUE);
        }
